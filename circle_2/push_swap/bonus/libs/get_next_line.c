...
/*   Updated: 2026/02/01 15:38:40 by mjabalqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*extract_line(char **buffer, char **pos, char **backup)
{
	...
	line = gnl_substr(*backup, 0, (size_t)(*pos - *backup + 1));
	if (!line)
		return (free(*buffer), NULL);
	temp = gnl_substr(*backup, (unsigned int)(*pos - *backup + 1),
			gnl_strlen(*pos + 1));
	if (!temp)
	{
		free(line);
