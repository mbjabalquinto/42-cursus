...
def ft_plant_types() -> None:
    ...
    for x in plants:
        print(f"{x}")
        if isinstance(x, Flower):
            print(f"{x.name.capitalize()} {x.bloom()}\n")
        elif isinstance(x, Tree):
            print(f"{x.name.capitalize()} {x.produce_shade()}\n")
        elif isinstance(x, Vegetable):
            print(
                    f"{x.name.capitalize()} "
                    f"is rich in {x.nutritional_value}\n"
            )
